...
export class DataProcessor {
  constructor() {
    ...
    this.currentThreshold = 20; // default threshold value (kept for backward compatibility)
    this.portThresholds = new Map(); // per-port threshold values
    this.currentDip = 0; // default dip value
  }

  updateSessionCSVData(round, stim, type, startTime, stopTime) {
    if (!this.sessionCSVData) {
      //Initialize the session csvData array if it does not exist.
      this.sessionCSVData = [];
    }
  ...
  updateSensorCSVData(id, state) {
    if (!this.sensorCSVData[id]) {
      //Initialize the csvData array if it does not exist.
      this.sensorCSVData[id] = [];
    }

    for (let i = 0; i < state.stimEDATime.length; i++) {
      //Cycle through the entire temp array.
  ...
  updateUnifiedCSVData(value, now, id, connectionManager, stimAnalyzer) {
    ...
    unifiedCSVDataRow.push(this.getPortThreshold(id)); // threshold (port-specific)
    ...
    if (
      this.currentPhase == "manual-offset" ||
      this.currentPhase == "manual-gain" ||
      this.currentPhase == "manual-threshold"
    ) {
  ...
  updateThreshold(threshold, portId) {
    this.currentThreshold = threshold; // Keep for backward compatibility
    if (portId) {
  ...
  getPortThreshold(portId) {
    return this.portThresholds.get(portId) || this.currentThreshold;
  }
