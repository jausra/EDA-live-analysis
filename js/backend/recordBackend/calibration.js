...
export class CalibrationManager {
    constructor(connectionManager, sessionManager, stimPresets, stimDisplay, roundManager, stimAnalyzer, dataProcessor) {
    ...
    setupCalibrationControls() {
        ...
        this.baselineADCTarget = 255*((1.00-this.desiredRange)/2);
        this.baselineADCTargetError = 0.05; // 0 to 1

        this.targetDeltaTolerance = 0.05
        this.targetDelta = 60;
        this.uppoerTargetDeltaBound = this.targetDelta * (1 + this.targetDeltaTolerance);
        this.lowerTargetDeltaBound = this.targetDelta * (1 - this.targetDeltaTolerance);
    ...
    calculateVoltageLimits() {
        ...
        this.desiredUpperVoltageMax = this.desiredUpperVoltage + (3.3 * this.desiredError);
        ...
        this.desiredUpperVoltageMin = this.desiredUpperVoltage - (3.3 * this.desiredError);
        ...
        this.desiredLowerVoltageMax = this.desiredLowerVoltage + (3.3 * this.desiredError);
        ...
        this.desiredLowerVoltageMin = this.desiredLowerVoltage - (3.3 * this.desiredError);
    ...
    async increaseRGainByRatio(ratio) {
        ...
        if (this.rGain > this.gainMax) { this.rGain = this.gainMax };
        if (this.rGain < this.gainMin) { this.rGain = this.gainMin };
    ...
    async msiHandleCalStimulus(round, presetName) {
        // if (round !== this.previousRound) {
        if (round == 2) {
            //We have just gotten to the correct offset level. 

            //Now, after the first round, detect the max and min, calculate delta from this. 

            //Have a desired delta. If the delta is not within the desired delta, amplify by changing rOff. 

            //For desired delta, also include some tolerance. 
            //Then run modify offset again and you should be good to go. 
            ...
            for (const [id, state] of this.connectionManager.getPortStates().entries()) {
                ...
                let delta = adcMax - adcMin;

                let ratio = this.targetDelta / delta;

                if (delta < this.lowerTargetDeltaBound || delta > this.upperTargetDeltaBound) {
                    await this.increaseRGainByRatio(ratio);
                }
                await this.modifyOffset(id, state);

                // //Check if ADC values are in range
                // if (adcMin >= this.desiredLowerADCMin && adcMin <= this.desiredLowerADCMax && adcMax >= this.desiredUpperADCMin && adcMax <= this.desiredUpperADCMax) { // If ADC values are in range, start the game
                // // if (true) {
                //     this.calculateTargetRMax(adcMin); // For printing out the value
                //     this.calculateTargetRMin(adcMax); // For printing out the value

                this.stimDisplay.running = false;
                this.stimDisplay.clearOnStimDisplay();
                // const stimWrapper = document.getElementById("stimWrapper");
                // stimWrapper.classList.toggle("hidden", true);
    ...
    msiInitCalibrationCB(presetName){
        ...
        //Function to be called every time we have a new stim. 
        this.stimDisplay.onStimDisplay(async ({ stim, round, color, startTime, stopTime }) => {
            await this.msiHandleCalStimulus(
                round,
                presetName
            );
        });
        ...
        while(!offsetCalDone) {
            ...
            if(state.stimEDAValues.length > 10){
                ...
                console.table({
                    ...
                    "target - err": this.baselineADCTarget - (255*this.baselineADCTargetError),
                    "target + err": this.baselineADCTarget + (255*this.baselineADCTargetError)
                });
                ...
                if (val < this.baselineADCTarget - (255*this.baselineADCTargetError)) {
                ...
                } else if (val > this.baselineADCTarget + (255*this.baselineADCTargetError)) {
                    if (shouldStep) {
                        this.rOff--;
                        if (this.dataProcessor) this.dataProcessor.updateOffset(this.rOff);
                        this.calOffsetSlider.value = this.rOff
                    ...
                    state.stimEDATime = [];
                    offsetCalDone = true;
                    if (this.stimAnalyzer) this.stimAnalyzer.endCalibrationSuppression(Date.now());
                    // this.hideCalibrationProgress();
                }
            }
        }
    ...
    calculateTargetR(adcOut) {
        const vOut = 3.3* (adcOut/255.00)
        // const rTarget = (this.rOff*(1.65*(this.rGain+10) - 10*vTarget)/(10*vTarget + 1.65*(this.rGain-10)));
        const rTarget = (this.rOff*(this.rGainLittle*(1.65-vOut) + 1.65*this.rGain))/((this.rGainLittle*(vOut-1.65)) + 1.65*this.rGain);
    ...
    calculateOffR() {
        ...
        this.rOff = Math.min(this.rOff, 4000.00);
        this.rOff = Math.max(this.rOff, 4.00);
    ...
    calculateGainR() {
        ...
        this.rGain = ((this.desiredLowerVoltage - 1.65)*this.rGainLittle)/(1.65-3.3*(this.rMax/(this.rMax+this.rOff)));
        // this.rGain = Math.min(this.rGain, 100.00);
        this.rGain = Math.min(this.rGain, 200.00);
    ...
    showBreathingCue(phase) {
        ...
        if (phase === 'calibration') {
            breathingText.innerHTML = 'Calibrating...<br>Take a slow deep breath<br>Exhale fast';
